...
export function PageHeader({ links = [], title, description, children, goBackLink }: PropsWithChildren<PageHeaderProps>) {
    return (
        <header className="border-b py-4 bg-secondary/50">
            <div className="mx-auto flex items-center justify-between gap-4 px-4 max-sm:flex-col max-sm:items-start">
                <div className="flex min-w-0 flex-1 flex-col items-start gap-2">
                    <PageHeaderBreadcrumb links={links} title={title} hidden={links.length === 0} />
                    <div className="flex items-center gap-2">
                        <Button asChild variant="outline" size="icon-xs" hidden={!goBackLink}>
